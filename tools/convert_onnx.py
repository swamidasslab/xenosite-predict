...
4. Updates ``name_tables.py`` (inlined column orders) when TSV headers exist.
...
def write_name_tables_py(tables: dict[tuple[str, str], tuple[str, ...]], dest: Path) -> None:
    """Rewrite ``name_tables.py`` with the given ``(model, head) → names`` map."""
    lines = [
...
def write_feature_names(model: str, head: str, names: list[str], dest: Path) -> None:
    """Update one ``(model, head)`` entry in ``name_tables.py``."""
    tables: dict[tuple[str, str], tuple[str, ...]] = {}
    if dest.is_file():
        ns: dict = {}
        exec(dest.read_text(encoding="utf-8"), ns)
        raw = ns.get("TABLES") or {}
        tables = {k: tuple(v) for k, v in raw.items()}
    tables[(model, head)] = tuple(names)
    write_name_tables_py(tables, dest)
        ...
        for head, rel in spec.get("tsv", {}).items():
            ...
            if cols and meta_path.is_file():
                ...
                if name == "ugt" and head == "atom":
                    ...
                    existing = load_names(name, head)
                    if existing:
                        cols = existing
                cols = cols[:n_in]
            if cols:
                write_feature_names(
                    name, head, cols, args.features_dir / "name_tables.py"
                )
