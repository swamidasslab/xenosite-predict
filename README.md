...
- **`rdkit=True`** keeps the in-process RDKit mols already built during parse and forest enumeration on `Molecule.rdkit` and `Metabolite.rdkit`. No extra parse. Omitted from JSON.
...
- Import does **not** open ONNX, HTTP, or OpenBabel. OpenBabel stays a required install for v0/v1; bindings load only when a v0/v1 ONNX predictor runs descriptors (not on `list_models` or HTTP). Callers never import `openbabel` / `pybel`.
