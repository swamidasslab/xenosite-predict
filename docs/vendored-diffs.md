...
- Quinone ortho/meta/para has two modes via ``quinone_omp_mode`` on ``predict(..., _parameter=)``:
  **legacy** (golden tests): one BFS shortest path per pair, neighbors in sorted atom-index order (deterministic port of vendored ``shortest_path``; fixes CPython 2.7 ``set`` hash tie-break). **principled** (API default): ``all_shortest_paths`` — true if *any* minimum-length path lies on an aromatic ring. The py2 dump oracle used hash-ordered BFS; quinone rows that disagree only on ``Ortho_``/``Meta_``/``Para_`` against that oracle are xfailed in ``test_ob_features`` (tier-1 oracle drift, not chemistry).
